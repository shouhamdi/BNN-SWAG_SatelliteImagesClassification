...
class SWAGInference(object):
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        # raw_predictions = predict_probabilities_swag(self, val_xs)
        # calibrated_probabilities = iso_reg.fit_transform(raw_predictions, val_ys)
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        bma_probabilities = torch.softmax(torch.stack(per_model_sample_predictions/2), dim=-1) #torch.mean(torch.stack(per_model_sample_predictions), dim=0)
